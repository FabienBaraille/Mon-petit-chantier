// Common
export const availableRowsPerPage = [10, 25, 50];
export const availableOrder = ['asc', 'desc'];

// Users
export const userRowsName = ["Email", "Role", "Statut"];
export const userRowsId = ['email', 'role', 'status'];

// Items
export const itemRowsName = ["Nom", "Rang", "Unité", "Statut"];
export const itemRowsId = ["name", "rank", "unit", "status"];

// Questions
export const questionRowsName = ["Titre", "Infos"];
export const questionRowsId = ["title", "infos"];

// Groups
export const groupRowsName = ["Titre", "Rang"];
export const groupRowsId = ["title", "rank"];
export const notSortableGroupRowsName = ["Questions"];

// Tips
export const tipsRowsName = ["Titre", "Description"];
export const tipsRowsId = ["title", "description"];
export const notSortableTipsRowsName = ["Référence"];